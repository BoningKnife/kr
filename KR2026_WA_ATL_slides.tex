...
\begin{frame}{Two Conceptual Ingredients}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{Harm and benefit}
      We follow a non-comparative intuition:
      \begin{itemize}
        \item harm: bringing about a bad outcome;
        \item benefit: bringing about a good outcome.
      \end{itemize}
      Good and bad outcomes are represented through agents' preferences.
    \end{block}

    \column{0.48\textwidth}
    \begin{block}{Danger}
      We follow a modal, non-probabilistic intuition:
      \begin{itemize}
  ...
  \large
  \[
    \text{strategic capability} + \text{preference over outcomes}
    \quad \Rightarrow \quad
    \text{welfare-affecting capability}
  \]
\end{frame}
...
\begin{frame}{Models: Concurrent Games with Preferences}
  ...
  \begin{itemize}
    ...
    \item a transition relation induced by joint actions;
...
\begin{frame}{The Game Structure}
  ...
  \begin{tikzpicture}[
    ...
    \node[state,label=below:$s_0$] (C) at (4,0) {$\mathit{inM}_0$};
    \node[state,label=below:$s_1$] (R) at (8,0) {$\mathit{inR}_0$};

    \path[->] (C) edge[loop above] node[above,labelbox] {$\delta_1,\delta_3,\delta_5,\delta_6$} ();
    \path[->] (R) edge[loop above] node[above,labelbox,text width=30mm,align=center] {$\delta_1,\delta_2,\delta_5,\delta_6,\delta_7,\delta_8$} ();
    \path[->] (L) edge[loop above] node[above,labelbox,text width=30mm,align=center] {$\delta_1,\delta_2,\delta_3,\delta_4,\delta_5,\delta_7$} ();
  ...
  \begin{tabular}{llll}
    $\delta_1=(0{:}\mathit{le},1{:}\mathit{bl},2{:}\mathit{bl})$ &
    $\delta_2=(0{:}\mathit{le},1{:}\mathit{bl},2{:}\mathit{fr})$ &
    $\delta_7=(0{:}\mathit{ri},1{:}\mathit{fr},2{:}\mathit{bl})$ &
    $\delta_8=(0{:}\mathit{ri},1{:}\mathit{fr},2{:}\mathit{fr})$
  \end{tabular}
  \end{adjustbox}

  \vspace{0.4em}
  The relevant strategic tension is: guards can jointly keep robot $0$ in the middle.
\end{frame}
...
\begin{frame}{Example: Blocking as Harm}
  Consider the strategy of coalition $\{1,2\}$:
  \[
    \js_{\{1,2\}}(\lambda) = (1{:}\mathit{bl},2{:}\mathit{bl})
    \quad\text{for every history }\lambda.
  \]

  \vspace{0.4em}
  Then at the initial state $s_0$:
  \[
    \js_{\{1,2\}} \in
    \SH_{\M_{\mathit{rs}}}^{\{1,2\}}(s_0,\{0\})
    \quad\text{and}\quad
    \js_{\{1,2\}} \in
    \SB_{\M_{\mathit{rs}}}^{\{1,2\}}(s_0,\{1,2\}).
  \]

  \vspace{0.5em}
  \begin{block}{Reading}
    The same strategy is harmful to robot $0$ and beneficial to the guarding coalition.
  \end{block}
\end{frame}
...
\begin{frame}{Reasoning About Changing Danger}
  Nested modalities let us express danger-enabling and danger-disabling choices.
  ...
  \begin{aligned}
  (\M_{\mathit{rs}},s_0)\models\;&
  \atlbh{\{0,1\}}{\{0\}}{\emptyset}\X\Danger(\{0\},\{1,2\})\\
  &\wedge\ \neg\Danger(\{0\},\{1,2\}).
  \end{aligned}
  \]
  ...
  \begin{block}{Intuition}
    Robot $0$ is not initially dangerous to the guards.
    But coalition $\{0,1\}$ can choose a beneficial strategy for robot $0$ that makes robot $0$ dangerous at the next state.
  \end{block}
\end{frame}
...
\begin{frame}[allowframebreaks]{Axiomatization}
  ...
  \begin{block}{Core axiom schemata}
    ...
    &\neg\atlbh{C}{B}{H}\X\top \quad \text{if } B\cap H\neq\emptyset &&(\text{BHC})
    \end{aligned}
    \]
    \end{small}
  \end{block}
  ...
  \begin{itemize}
    ...
    \item The context tracks the welfare-affecting modality currently being realized.
  \end{itemize}
\end{frame}

\begin{frame}{Why a Contextual Canonical Model?}
  Standard ATL-style canonical constructions are not enough because:
  \[
    \atlbh{C}{B}{H}\gamma
  \]
  constrains both strategic outcomes and welfare status.

  \vspace{0.6em}
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
    \begin{block}{Strategic part}
      The canonical action must realize the temporal guarantee $\gamma$.
    \end{block}

    \column{0.5\textwidth}
    \begin{block}{Preference part}
      The same action must generate best computations for $B$ and worst computations for $H$.
    \end{block}
  \end{columns}

  \vspace{0.8em}
  \centering
  Formula contexts synchronize these two requirements in the canonical construction.
\end{frame}
