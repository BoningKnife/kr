...
formula}
  Let $\Omega \subseteq \St^\psi$ and $\chi \in \ecl(\phi)$, we say $\chi$ is a characteristic formula of $\Omega$ iff for all $s\subseteq \St^\psi$ we have $s \vdash \chi $ iff $s\in \Omega$.
\end{definition}

\begin{theorem}
    Every $\Omega \subseteq \St^\psi$ has a characteristic formula.
\end{theorem}
...
For every canonical states $s\in \St^\psi$ and every aciton profile $\ja : \AGT \to \ACT^{\psi }$, define $s' $ is a candidate successors of $\ja$ at $s$, iff:
\begin{itemize}
    \item for all $\atlbh{\Coalition}{B}{H}\nexttime \chi\in \Consesus(\ja)$, if $s\vdash \atlbh{\Coalition}{B}{H}\nexttime \chi$, then $s'\vdash \chi$;
    \item for all $\chi\in \ecl(\psi)$, if $s\nvdash \atlbh{\AGT}{\emptyset}{\emptyset}\nexttime \chi$, then $s'\vdash \neg \chi$.
\end{itemize}
