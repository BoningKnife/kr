...
\newcommand{\Ylnote}[1] { \footnote{\textcolor{red}{\textbf{YL}: #1} }}
...
the welfare of  another coalition when the former brings benefit (resp.\ harm) to the latter through the choice of a particular strategy. 
...
%           \endgroup
\end{definition}
Property $\mathbf{SP}$ expresses stability of preferences over time. 
It captures the idea that an agent's preference remains consistent over time: an agent prefers a computation $\lambda$ to a computation $\lambda'$ starting at the same state $s'$ if and only if it prefers the precursor of $\lambda$ (i.e., $s\lambda$) to the precursor of $\lambda'$ (i.e., $s\lambda'$) at every predecessor $s$ of $s'$.
It  is in line with the notion of \emph{time consistency} of preferences studied in economics, as opposed to \emph{time inconsistency} \cite{Lowewenstein2002}. 
Property $\mathbf{NFP}$ ensures agents are not indifferent among all outcomes. Properties $\mathbf{WFP}$ and $\mathbf{CWFP}$ guarantee, respectively, the existence of worst and best outcomes. 
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \fbox{\begin{minipage}{0.9\linewidth}\vspace{-1em}
        \[
        \begin{array}{ll}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \fbox{\begin{minipage}{0.70\linewidth}\vspace{-1em}
        \begin{align*}
...
In the semantic interpretation of the welfare-affecting strategic capability modalities we will introduce in Section \ref{sec:languages}, we will only use the strong notions, since our axiomatization and embedding into standard $\atllogic$ rely on the notions of best and worst outcomes (whose existence is guaranteed by the regularity assumption on preferences). However, the weak notions are provided here for two reasons: to show that there is no unique way to define beneficial or harmful strategy, and to make the conceptual analysis of beneficial/harmful strategies richer. 
...
A pointed model is a pair consisting of a CGMRP and one of its states, when interpreting a state formula, or a pair consisting of a CGMRP and one of its computations, when interpreting a path formula. 
...
This choice is motivated by our technical results: strong benefit and harm are tailored to best and worst outcomes, and they are the notions for which we provide an axiomatization and obtain an embedding into standard $\atllogic$ under regular preferences. 
...
\begin{align*}
(\M_{\mathit{rs}}, s_0)\models & \atlbh{\{0,1\}}{\{0\}}{\emptyset}\nexttime \ \mathsf{Danger}(\{0\},\{1,2\})
\ \wedge \\
...
We conclude this section by defining two   fragments of the language $\lang_{\atlBDlogic}$. 
...
\begin{itemize}
    ...
    \vdash \bigwedge_{\phi \in s.1} \phi\rightarrow \atlbh{\Coalition}{B}{H}\nexttime \chi$, then $  \vdash \bigwedge_{\phi \in s'.1 } \phi \rightarrow \chi$, $B\subseteq s'.2$, and $H\subseteq s'.3$;
    \item for all $\chi\in \ecl(\psi)$ and $B,H\in \powerset{\AGT}$, if $ \nvdash \bigwedge_{\phi \in s.1} \phi \rightarrow \atlbh{\AGT}{B}{H}\nexttime \chi$, then $\vdash \bigwedge_{\phi \in s'.1 } \phi \rightarrow \neg \chi$, $B\nsubseteq s'.2$, or $H\nsubseteq s'.3$.
\end{itemize}
...
\begin{definition}[Canonical CGM]\label{def: canonicalCGM}
  Given a formula $\psi $, define the canonical concurrent game model as the  structure $\CM^\psi = (\St^{\psi }, \ACT^{\psi }, \R^\psi, \V^\psi)$ such that: 
  \begin{itemize}
...
\begin{definition}[Utility function in canonical model]
  Given a canonical CGM $\CM^\psi = (\St^{\psi }, \ACT^{\psi }, \R^\psi, \V^\psi)$, for each agent $a$ and canonical state $s$, we define a utility function $U_{a,s}: \Comp_{\M^\psi, s} \to \mathbb{Z}\cup \{-\infty, +\infty\}$ such that for any computation $\history \in \Comp_{\CM^\psi, s}$: 
\[
U_{a,s}(\history) = 
\begin{cases} 
+\sup(\mathbb{B}) & \text{if } a\in \history(1).2,\\
-\sup(\mathbb{H}) & \text{if } a\in \history(1).3,\\
0 & \text{otherwise.}
\end{cases}
\]
where $\sup$ is the supremum function, $\mathbb{B}=\{k\in \mathbb{N}\mid  \forall 0< k'\leq k: a\in \history(k').2\}$, and $\mathbb{H}=\{k\in \mathbb{N}\mid  \forall 0< k'\leq k: a\in \history(k').3\}$. Note $s'.2\cap s'.3=\emptyset$ for every $s'\in \St^\psi$, so $U_{a,s}$ is well-defined.
\end{definition}
Now we can define the canonical CGMRP. 
\begin{definition}[Canonical CGMRP]\label{def: canonicalmodel}
  Given a formula $\psi $, define the canonical CGMRP as the structure $\M^\psi = (\St^{\psi }, \ACT^{\psi }, \R^\psi, \preceq^\psi, \V^\psi)$ such that:
  \begin{itemize}
    \item $(\St^{\psi }, \ACT^{\psi }, \R^\psi, \V^\psi)$ is the canonical CGM defined in Definition \ref{def: canonicalCGM};
    ...
    such that for all $\history,\history'\in \historyset_{\M^\psi,s}$:\\
     $\history \preceq^\psi_{a,s} \history'$ iff $U_{a,s}(\history) \leq U_{a,s}(\history')$.
  \end{itemize} 
\end{definition}
...
% The proof is given in section \ref{sec:app:completeness} of the supplementary material. 
...
The tree-like model property is stated in the following lemma. 
% Its proof is given in section \ref{sec:app:tree_like} of the supplementary material. 
...
As the following theorem shows, by using the translation $ \mathit{tr}$, satisfiability for the language $\lang_{\atlBDlogic}$ is reducible to $\atllogic$-satisfiability, and satisfiability for the fragment $\lang_{\clBDlogic}$ is reducible to $\cllogic$-satisfiability. 
% Its proof is given in section \ref{sec:app:embedding} of the supplementary material. %\cite{ArxivAAMAS26}. %in the supplementary material.
 \begin{theorem}\label{theo:embedding}
     ...
     iff
     \begin{align*}
         \mathit{tr}(\varphi) \wedge \bigwedge_{a\in\AGT}(\atlop{\AGT}\X \help_a\wedge
     \atlop{\AGT}\X\harm_a)\wedge 
      \\
      \atlop{\emptyset }\G  \big(\bigwedge_{a\in\AGT}(\atlop{\AGT}\X  \help_a\wedge\atlop{\AGT}\X\harm_a \wedge \\
       \neg(\help_a\wedge\harm_a) )\big)
     \end{align*}
     % $\mathit{tr}(\varphi) \wedge \bigwedge_{a\in\AGT}(\atlop{\AGT}\X \help_a\wedge
     % \atlop{\AGT}\X\harm_a)\wedge \atlop{\emptyset }\G\big(\bigwedge_{a\in\AGT}(\atlop{\AGT}\X  \help_a\wedge\atlop{\AGT}\X\harm_a \wedge \neg(\help_a\wedge\harm_a) )\big)$
     is satisfiable;
     % \elnote{
     ...
     iff 
       \begin{align*}
\mathit{tr}(\psi) \wedge \bigwedge_{a\in\AGT}
     \big(\atlop{\AGT}\X  \help_a\wedge\atlop{\AGT}\X\harm_a\wedge\\
     \atlop{\emptyset }\X\neg(\help_a\wedge\harm_a) \big)
         \end{align*} 
     % $\mathit{tr}(\psi) \wedge \bigwedge_{a\in\AGT}
...
This work is   supported by the TIRIS project CaRe (Caring about Others: AI and Psychology Meet to Model and Automate Collective
Reasoning) 
and the ANR PRCE project DesiRes (Designing Artificial Reasoners for Communication). 
