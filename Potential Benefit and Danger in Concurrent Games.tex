...
\begin{definition}[MATF]\label{def: MATF}
    ...
    \item $\R: \ACT^{\AGT}\to \powerset{\St\times \St}$ assign a transition relation on $\St$ to every action profile, where $\ACT^{\AGT}=\{\ja_{\AGT}
    \suchthat \ja_{\AGT}:
...
since in set theory
a function is representable as a  set of ordered pairs.
...
%           \endgroup
\end{definition}
Property $\mathbf{SP}$, expressing \emph{stability} of preferences, captures the idea that an agent's preference remains consistent over time: an agent prefers a computation $\lambda$ to $\lambda'$ starting at the same state $s'$ if and only if it prefers the precursor of $\lambda$ (i.e., $s\lambda$) to the precursor of $\lambda'$ (i.e., $s\lambda'$) at every predecessor $s$ of $s'$. 
Stability of preferences aligns with the notion of \emph{time consistency} studied in economics, as opposed to \emph{time inconsistency} \cite{Lowewenstein2002}.  
Property $\mathbf{NFP}$, expressing \emph{non-flatness} of preferences, ensures that agents are not indifferent among all outcomes.  
Properties $\mathbf{WFP}$ and $\mathbf{CWFP}$, expressing \emph{well-foundedness} and \emph{converse well-foundedness} of preferences, guarantee that agents' preferences have both a worst and a best outcome.


In the remainder of the paper, we focus exclusively on CGFs with regular preferences. 
...
assume a set a countably infinite  set of atomic
propositions $\AP$
...
The proof is in 
Appendix \ref{sec: Proof of Proposition 1} of the supplementary material.
...
$\ourlogic$
(Logic of Welfare-Affecting Strategic Capabiliy) 
...
Given an formula $\psi $, we denote the set of all subformulas of $\psi $ by $\sub(\psi)$. Let $\sub^+(\psi ) = \{\top, \bot \}\cup \sub(\psi)\cup 
...
The following definition
introduces
the picking function for canonical actions. 
\begin{definition}[Picking]
  Given a canonical action   $(\atlbh{\Coalition}{B}{H}\nexttime \chi , a , \choicef)\in \ACT^{\psi }$,
  we define 
  \begin{align*}
      \Fsupport\big((\atlbh{\Coalition}{B}{H}\nexttime \chi , a , \choicef) \big) & = \atlbh{\Coalition}{B}{H}\nexttime \chi , \\
      \vote\big((\atlbh{\Coalition}{B}{H}\nexttime \chi , a , \choicef)\big) & = a ,\\
...
Given a canonical state $s=(X,B,H)$ and action profile $\ja : \AGT \to \ACT^{\psi }$,
we call  $s'=(X',B',H') $ the candidate successors of $\ja$ at $s$, iff:
\begin{itemize}
    \item for all $\atlbh{\Coalition}{B''}{H''}\nexttime \chi\in \Consesus(\ja)$, if $
    \vdash \bigwedge_{\phi \in X} \phi\rightarrow \atlbh{\Coalition}{B''}{H''}\nexttime \chi$, then $  \bigwedge_{\chi \in X' } \chi\vdash \chi$, $B''\subseteq B'$, and $H''\subseteq H'$;
    \item for all $\chi\in \ecl(\psi)$ and $B'',H''\in \powerset{\AGT}$, if $ \nvdash \bigwedge_{\phi \in X} \phi \rightarrow \atlbh{\AGT}{B''}{H''}\nexttime \chi$, then $X'\vdash \neg \chi$, $B''\nsubseteq B'$, or $H''\nsubseteq H'$.
\end{itemize}
...
\Big ( \big(\sum_{a\in\AGT}\vote(\ja(a))\big)\bmod n\Big ) +1$, where $n=|\AGT|$. The agent $\te(\ja)$ is called the elected agent of
...
\begin{definition}[Canonical model]\label{def: canonicalmodel}
  ...
  \begin{itemize}
    ...
    $\R_\ja $ on $\St$ such that for all $s,s'\in\St^\psi$ and $\ja : \AGT \to \ACT^{\psi }$, we have $s\R_\ja s'$ iff $s' = \de\big(\ja(a) \big)\big(\CS(s,\ja) \big)$, with  $a = \te(\ja)$;
    \item $\preceq^\psi $ assigns to
    each agent-state pair
    $(a,s)$ a total preorder on computations starting at  $s$
    such that  for all  $\history,\history'\in \historyset_{\M^\psi,s}$, $\history \preceq^\psi_{a,s} \history'$ iff there are $(X,B,H),(X',B',H')\in \St^\psi$ such that $(X,B,H)=\history(1)$, $(X',B',H')=\history'(1)$ and either $a\notin (B\setminus B') $ or $a\notin (H'\setminus H)$;
...
% \elnote{Please check the new names
% of Lemma/Theorems. Lemma 1 was previously Theorem 1. In the appendix please name each section with corresponding Proposition/Lemma/Theorem. }
...
As the following theorem shows, by using the translation $ \mathit{tr}$, satisfiability for the language $\lang_{\atlBDlogic}$ is reducible to $\atllogic$-satisfiability, and satisfiability for the fragment $\lang_{\clBDlogic}$ is reducible to $\cllogic$-satisfiability.
     Its proof 
     is given in Appendix \ref{sec: embeding perserve satifability} of the supplementary material. %\cite{ArxivAAMAS26}. %in the supplementary material.
