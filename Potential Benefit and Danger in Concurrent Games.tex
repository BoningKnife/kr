...
% We respectively denote by $\succeq_{i,s}$ and $\succ_{i,s}$ the inverses of $\preceq_{i,s}$ and $\prec_{i,s}$.
       ...
       (\mathbf{SP}) & \forall s,s' \in \St ,
        ...
        s \lambda'   \preceq_{i,s } s\lambda 
          ...
          &\forall \in \history' \in  \historyset_{\F,s}:\history \preceq_{i,s} \history' .\\
          (\CWFP)\  &\forall s\in \St, \exists \history \in \historyset_{\F,s}\text{ such that }\\
          & \forall \in
          \history' \in \historyset_{\F,s}:\history' \preceq_{i,s} \history.
          \end{align*}
        we say that the CGFP 
          $(\F,\preceq)$ has regular preferences iff it has stable, non-flat, well-founded, and converse-well-founded preferences.
        \endgroup
\end{definition}
...
\end{definition}
\begingroup\color{red}
In Definition~\ref{def: CGFP}, we introduced four properties for CGFPs. 
These properties are inherited by CGMPs. The CGMPs with all four properties are called having \emph{regular preferences} ($\mathbf{RP}$).
we denote by $\classcgspvar{X}$ the class of CGMPs satisfying all properties in $X$. 
For example, $\classcgspvar{\{\RP\}}$ denotes the class of CGMPs with regular preferences, 
  while $\classcgspvar{\emptyset}$ denotes the class of all CGMPs. We will focus on the class $\classcgspvar{\{\RP\}}$ in later sections.
\endgroup
...
Note we use the temporal operator $\G$ and $\U$ referring to the strict future, i.e., starting from the next state. But the temporal operators referring to the non-strict future can be defined as usual:
\begin{align*}
...
where  $p$ ranges over  $\AP$ and $\Coalition $, $\GroupB$ and $\GroupH $ range over $2^\AGT$. 
 ...
 $\lang_{\atlBDlogic} $ and its fragments in combination with  with the different CGMP classes give rise to a rich family of logics.
In this paper, we focus on the logics on the CGMP class $\classcgspvar{\{\RP\}}$.
\endgroup
...
In this section, we offer axiomatization for the logic of the \emph{safety fragment} ($\lang_{\atlBDlogic^{-\U}}$) on the CGMP class $\classcgspvar{\{\RP\}}$, which includes all CGMPs with regular (stable, well-founded, converse-well-founded and non-flat) preferences.

\subsection{axiomatization}\label{subsec: axiomatization}

The following definition
introduces the xiomatic system $\Lambda$.
\begin{definition}[axiomatic system $\Lambda$]
\label{definition: axiomatic system}
$\Lambda$ consists of the following axioms:
\begin{align*}
& 
...
\\
& 
\atlbh{\Coalition}{B}{H}\G \phi \to  \atlbh{\Coalition}{B}{H}\nexttime (\phi \wedge \atlbh{\Coalition}{B}{H}\G \phi)
\tag{$\mathtt{A}\text{-}\mathtt{FP}_\G$}\label{ax:FP_G}
\\
& 
\atlop{\emptyset}\G\big (\psi \to \atlbh{\Coalition}{B}{H}\X(\phi \wedge \psi)\big ) \to  \atlop{\emptyset }\G (\psi \to \atlbh{\Coalition}{B}{H}\G \phi)
...
since it corresponds to the fact that the effectivity function of a game is a crown. 
\begingroup\color{red}
Axiom \ref{ax:FP_G} is the \textit{fixpoint axiom for $\G$}, which captures the fixpoint character of the operator $\G$. Axiom \ref{ax:GFP_G} is the \textit{greatest fixpoint axiom for $\G$}, which reflects the greatest post-fixpoint character of the operator $\G$. Axioms \ref{ax:WFP} and \ref{ax:CWFP} capture the \textit{well-foundedness} and \textit{converse well-foundedness} of preferences, respectively. Finally, Axiom \ref{ax:BHC} captures the \textit{benefit-harm consistency}: a coalition cannot have a strategy that is both beneficial and harmful for the same group at the same time, which is related to the property that the preferences of agents are non-flat.
The inference rules are \textit{modus ponens} (\ref{ir:MP}), \textit{replacement of equivalences} (\ref{ir:RE}) and \textit{generalization} (\ref{ir:Gen}).
\endgroup
...
is  an instance of one of the  axiom schema of  $\Lambda$
...
is an instance of some inference  rule of $\Lambda$.
\end{itemize}

It is straightforward to verify that all axioms of $\Lambda$ are valid and all inference rules of $\Lambda$ preserve validity in the class $\classcgspvar{\{\RP\}}$. So, soundness of $\Lambda$ follows. For completeness, we use a technique of canonical models under formula contexts.

\subsection{canonical models under formula contexts}
 We are going
 to prove completeness by canonical models under formula contexts.
...
\begin{theorem}
Given a consistent formula $\psi \in \lang_{\atlBDlogic^{-\U}}$, there is a state $s\in \St^\psi$ in the canonical model $\M^\psi$ such that $\M^\psi,s \models \psi$.
\end{theorem}
...
In the completeness proofs for the logics $\Lambda$ and $\safyatlNFSBDlogic$, we construct a tree model for every satisfiable formula. 
