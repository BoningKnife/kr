...
\begingroup\color{red}
\begin{enumerate}
  \item the one based on the \emph{safety fragment} ($\lang_{\atlBDlogic^{-\U}}$), and the model class $\classcgspvar{\{\SP\}}$, which includes all CGMPs with stable preferences;
  \item the one based on the  full language $\lang_{\atlBDlogic}$, and the model class $\classcgspvar{\{\SP\}}$, which includes all CGMPs with stable preferences;
\end{enumerate}
We offer axiomatizations for the first logic in Section~\ref{sec:axiom}, and proof of tree-like property and embedding results for the second logic in Section~\ref{sec: tree-like model property and embedding}. 
\endgroup
