...
\begin{align*}
    \atlbh{\Coalition}{B}{H}\G^\weak\varphi \eqdef&   \varphi\wedge\atlbh{\Coalition}{B}{H} \G\varphi,\\
 \atlbh{\Coalition}{B}{H}(\varphi \U^\weak \psi )\eqdef& \psi \vee (\varphi \wedge \atlbh{\Coalition}{B}{H} (\varphi \U \psi )) .   
\end{align*}
also that standard $\mathsf{ATL}$
modalities
$\atlop{\Coalition }\nexttime\varphi$,
$\atlop{\Coalition }\henceforth^\weak \phi$ and
$\atlop{\Coalition }(\varphi \U^\weak \psi )$
are definable
as special cases
...
\begin{align*}
    ...
    \atlop{\Coalition }\henceforth^\weak\varphi  \eqdef&
 \atlbh{\Coalition}{\emptyset}{\emptyset  }\henceforth^\weak\varphi  ,\\
    \atlop{\Coalition }(\varphi \U^\weak \psi )\eqdef& \atlbh{\Coalition}{\emptyset}{\emptyset  } (\varphi \U^\weak \psi ) .   
\end{align*}
...
(i) the one based on the \emph{safety fragment} ($\lang_{\atlBDlogic^{-\U}}$), and the general model class $\classcgspvar{\emptyset}$, which includes all CGMPs; and
(ii) the one based on the \emph{safety fragment} ($\lang_{\atlBDlogic^{-\U}}$), the satisfaction relation $\models$, and the model class $\classcgspvar{\{\NFP\}}$, which includes all CGMPs with non-flat preferences.
We call them logics $\safyatlSBDlogic$ 
and $\safyatlNFSBDlogic$.
